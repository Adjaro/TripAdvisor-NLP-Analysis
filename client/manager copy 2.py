...
def read_restaurant(db: Session, skip: int = 0, limit: Optional[int] = None) -> pd.DataFrame:
    try:
        query = db.query(models.DimRestaurant).offset(skip)
...
def read_location(db: Session, skip: int = 0, limit: Optional[int] = None) -> pd.DataFrame:
    try:
        query = db.query(models.DimLocation).offset(skip)
        if limit is not None:
            query = query.limit(limit)
        locations = query.all()
...
def read_date(db: Session, skip: int = 0, limit: Optional[int] = None) -> pd.DataFrame:
    try:
        query = db.query(models.DimDate).offset(skip)
        if limit is not None:
            query = query.limit(limit)
        dates = query.all()
...
def read_review(
    ...
    try:
        query = db.query(models.FaitAvis).offset(skip)
